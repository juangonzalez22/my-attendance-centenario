...
export default function Home() {
  ...
  const handleRegistroAsistencia = async (event) => {
    ...
    if (!idIngresado) {
      ...
      errorSound.play(); // Sonido de error
      return;
    }
    ...
    if (asistenciaHoy.length > 0) {
      setAdvertencia(`⚠️ El estudiante ${estudianteData.nombre} del grupo ${estudianteData.grupo} ya fue registrado hoy.`);
    }
    ...
    if (insertError) {
      ...
      setMensaje(`✅ Asistencia registrada para ${estudianteData.nombre} del grupo ${estudianteData.grupo} a las ${obtenerHoraLocal(fechaISO)}.`);
      registeredSound.play(); // Sonido de registro exitoso
  ...
  <head>
    ...
    <style>
      /* Estilos inline que funcionen en la mayoría de los clientes de correo */
      .container {
  ...
  <body>
    <div class="container">
      <div class="content">
        <div class="header">
          Notificación de Llegada al Colegio
        </div>
        <div class="body">
          <p>Estimado/a padre/madre/acudiente,</p>
          <p>
            Le informamos que <strong>${estudianteData.nombre}</strong> del grupo <strong>${estudianteData.grupo}</strong> ha llegado al colegio a las <strong>${obtenerHoraLocal(fechaISO)}</strong>.
          </p>
          <p>Saludos cordiales,<br/>Institución Educativa Centenario</p>
        </div>
        <div class="footer">
          © ${new Date().getFullYear()} Institución Educativa Centenario. Todos los derechos reservados.
          <p>
              Con la tecnología de Velvet Attendance, desarrollada por Velvet Digital.
          </p>
        </div>
      </div>
    </div>
      ...
      // Llamada al endpoint para enviar el correo con el HTML
      ...
      await fetch("/api/agregarAsistencia", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleEliminarUltimoRegistro = async () => {
    ...
    if (error) {
      ...
      setMensaje(`✅ Se ha eliminado el último registro del estudiante ${ultimoRegistro.nombre} del grupo ${ultimoRegistro.grupo}.`);
      registeredSound.play(); // Sonido de registro exitoso
  ...
  return (
    <PasswordGate>
      {/* Contenedor principal con la misma estructura que Menú Principal */}
      <div
      ...
      >
        ...
        <div className="relative z-10 w-full flex flex-col items-center">
          ...
          {mensaje && (
            <motion.p
              className="mb-2 text-green-200 font-semibold"
          ...
          {advertencia && (
            <motion.p
              className="mb-2 text-yellow-300 font-semibold"
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.3 }}
            >
          ...
          {ultimoRegistro && (
            <motion.button
              onClick={handleEliminarUltimoRegistro}
              className="bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-lg shadow-md mt-6 transition duration-300"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
